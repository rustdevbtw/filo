...
void WebviewHandler::OnFaviconURLChange(CefRefPtr<CefBrowser> browser,
                                const std::vector<CefString>& icon_urls) {
    //todo: favicon change
    if(onFaviconURLChangedEvent) {
        onFaviconURLChangedEvent(browser->GetIdentifier(), icon_urls.at(0));
    }
}
