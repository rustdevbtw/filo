...
namespace webview_cef {
	...
	void WebviewPlugin::initCallback() {
		if (!m_init)
		{
            ...
            m_handler->onFaviconURLChangedEvent = [=](int browserId, std::string faviconUrl)
            {
                if (m_invokeFunc)
                {
                    WValue* bId = webview_value_new_int(browserId);
                    WValue* wFaviconUrl = webview_value_new_string(const_cast<char*>(faviconUrl.c_str()));
                    WValue* retMap = webview_value_new_map();
                    webview_value_set_string(retMap, "browserId", bId);
                    webview_value_set_string(retMap, "faviconURL", wFaviconUrl);
                    m_invokeFunc("faviconURLChanged", retMap);
                    webview_value_unref(bId);
                    webview_value_unref(wFaviconUrl);
                    webview_value_unref(retMap);
                }
            };
	...
	void WebviewPlugin::uninitCallback(){
        ...
        m_handler->onFaviconURLChangedEvent = nullptr;
